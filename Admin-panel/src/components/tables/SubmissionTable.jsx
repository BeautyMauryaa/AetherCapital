...
export default function SubmissionTable({ submissions, title, isApprovedPage, filterState }) {
  ...
  // Shared UI component for empty placeholder states
  const renderEmptyState = (message = "No data yet") => (
    <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" gap={1.5} py={6}>
      <Box 
        sx={{ 
          display: 'flex', 
          alignItems: 'center', 
      ...
      >
        <ManageSearchRounded sx={{ fontSize: 26 }} />
      </Box>
      <Box textAlign="center">
        ...
        <Typography variant="caption" sx={{ color: "#6b7280" }}>
          There are currently no submissions matching this panel view.
        </Typography>
      </Box>
    </Box>
  );
  ...
  return (
    <>
      <Card sx={{ borderRadius: "16px", boxShadow: "0px 1px 3px rgba(0,0,0,0.05)", border: "1px solid #f3f4f6", bgcolor: "#fff" }}>
        ...
        {/* ── MOBILE VIEW ── */}
        <Box sx={{ display: { xs: "block", sm: "none" }, px: submissions.length === 0 ? 3 : 0 }}>
          {submissions.length === 0 ? (
            renderEmptyState("No data yet")
          ) : (
