    ...
    (set, get) => ({
      ...
      submitApplication: async () => {
        ...
        try {
          ...
          // Append files from binary fileStore
          ["profileImage", "idFront", "idBack"].forEach((key) => {
            const file = fileStore.get(key);
            if (file instanceof File) fd.append(key, file);
          });
          ...
          // Clean text payload assembly (Fixes the double-stringification bug)
          const textData = {};
          for (const [key, val] of Object.entries(formData)) {
            if (
              key.endsWith("__name") ||
            ...
            // Maintain objects and arrays cleanly as objects; stringify happens once globally below
            textData[key] = val;
          }
          if (signatureData) textData.signatureData = signatureData;

          // Package everything cleanly as a single serialized object string
          fd.append("formData", JSON.stringify(textData));
