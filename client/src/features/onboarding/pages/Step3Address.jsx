...
const {
  ...
  const [showTimezoneDrop, setShowTimezoneDrop] = useState(false);
  ...
  return (
    <div className="max-w-4xl mx-auto px-2 pt-10 pb-28">
      ...
      <div className="mb-5">
        ...
        {/* Updated Country Dropdown List */}
{showCountryDrop && (
  <div className="absolute z-[999] bottom-[calc(100%+8px)] left-0 w-full max-h-60 overflow-y-auto
    bg-[#121214] border border-white/10 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,1)] 
    backdrop-blur-none">
    ...
    {errors.zip && (
      ...
      <div className="mb-8">
        ...
        <div className="relative">
          <button
            type="button"
            onClick={() => setShowTimezoneDrop((o) => !o)}
            className="w-full h-[52px] px-4 rounded-xl bg-card border border-border
              flex items-center justify-between text-[14px] text-foreground
              hover:border-foreground/20 focus:outline-none focus:border-purple-500/50
              transition-all duration-200"
          >
            <span>{timezone}</span>
            <ChevronDown
              ...
              className={`text-foreground/30 transition-transform duration-200
                ${showTimezoneDrop ? "rotate-180" : ""}`}
            />
          </button>

          {/* Updated Timezone Dropdown List */}
{showTimezoneDrop && (
  <>
    {/* Click-away backdrop */}
    <div className="fixed inset-0 z-[998]" onClick={() => setShowTimezoneDrop(false)} />
