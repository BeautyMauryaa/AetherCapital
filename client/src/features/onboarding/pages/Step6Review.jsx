// // src/features/onboarding/pages/Step6Review.jsx
// // FIX 2: Submit errors shown as inline UI component — no alert().
// // FIX 4: Signature preview always shows from formData.signatureData.
// // FIX 5: Download receipt button wired to store.generateReceipt().
...
// import { Building2, User, ShieldCheck, AlertCircle, Download, X } from "lucide-react";
...
//   const submitError       = useOnboardingStore((s) => s.submitError);      // FIX 2
...
//     // Check Drive URL first (uploaded via separate API — FIX 3)
//     if (formData[`${fieldName}__driveUrl`]) return `✓ ${formData[`${fieldName}__name`] || "uploaded"}`;
//     // Fallback to name
...
//     const uploadedDocs = Object.values(docs).filter((d) => d?.driveUrl);
//     if (uploadedDocs.length > 0) return `${uploadedDocs.length} file(s) uploaded`;
//     return "None uploaded";
//   };

//   const profilePreviewUrl = formData.profileImage__preview || null;
...
//       <div className="mt-12 overflow-hidden rounded-[32px] shadow-sm transition-all duration-300"
...
//       {/* ── FIX 4: Signature preview — always visible when signatureData exists ── */}
//       {signatureData && (
...
//           {/* FIX 5: Download receipt button */}
//           {isSubmitted && (
//             <button
//               onClick={generateReceipt}
//               className="mt-4 flex items-center gap-2 px-4 py-2 rounded-xl border border-purple-500/30 bg-purple-500/5 text-purple-500 text-[12px] font-semibold hover:bg-purple-500/10 transition-all"
//             >
//               <Download size={14} />
...
//         {/* FIX 2: Inline validation — replaces alert() entirely */}
...
//         {/* FIX 2: Submit error — inline UI component, never alert() */}
//         {submitError && (
