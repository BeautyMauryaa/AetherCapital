...
// ── Zip validation ──────────────────────────────────────────────────────────
const ZIP_PATTERNS = {
  "United States": { pattern: /^\d{5}(-\d{4})?$/, hint: "e.g. 10001" },
  "United Kingdom": { pattern: /^[A-Za-z]{1,2}\d[A-Za-z\d]? ?\d[A-Za-z]{2}$/, hint: "e.g. SW1A 1AA" },
  "Canada":         { pattern: /^[A-Za-z]\d[A-Za-z] ?\d[A-Za-z]\d$/, hint: "e.g. K1A 0B1" },
  "Australia":      { pattern: /^\d{4}$/, hint: "e.g. 2000" },
  "India":          { pattern: /^\d{6}$/, hint: "e.g. 110001" },
  "Germany":        { pattern: /^\d{5}$/, hint: "e.g. 10115" },
  "Singapore":      { pattern: /^\d{6}$/, hint: "e.g. 018989" },
};

const validateZip = (value, country) => {
...
const Step3EnterPriseAdd = () => {
  ...
  const handleValidateAndNext = () => {
    ...
    ["country", "address1", "city", "state", "zip"].forEach((field) => {
      if (!formData[field]?.trim()) newErrors[field] = "Required";
    });

    // Zip format check
    if (!newErrors.zip) {
      const zipError = validateZip(formData.zip || "", formData.country || "");
  ...
  return (
    <div className="max-w-4xl pb-24 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                {showCountryDrop && (
                  <div className="absolute z-50 top-full mt-1 w-full rounded-xl overflow-hidden shadow-2xl max-h-64 overflow-y-auto"
                      ...
                      <div
                        ...
                        onClick={() => {
                          ...
                          if (formData.zip) {
                            setErrors((prev) => ({
                              ...prev,
                              zip: validateZip(formData.zip, country),
                            }));
                          }
                        }}
                        className="px-5 py-3 cursor-pointer transition-colors text-[13px]"
              ...
              {errors.country && (
                <span className="text-[10px] text-red-500 ml-1 font-bold tracking-wider">! REQUIRED</span>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              />
              {errors.address1 && (
                <span className="text-[10px] text-red-500 ml-1 font-bold tracking-wider">! FIELD REQUIRED</span>
              )}
            </div>
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setErrors((prev) => ({
                      ...prev,
                      zip: validateZip(e.target.value, formData.country || ""),
                    }));
                  }}
                  onBlur={() => {
                    setErrors((prev) => ({
                      ...prev,
                      zip: validateZip(formData.zip || "", formData.country || ""),
                    }));
                  }}
                  className="w-full h-[56px] px-5 rounded-2xl focus:outline-none focus:ring-1 focus:ring-purple-500/50 transition-all"
                ...
                />
                {errors.zip && (
                  <span className="text-[10px] text-red-500 ml-1 font-bold tracking-wider">
                    {errors.zip}
                  </span>
                )}
                {!errors.zip && ZIP_PATTERNS[formData.country] && (
                  <p className="text-[10px] italic ml-1" style={{ color: "var(--text-main)", opacity: 0.25 }}>
        ...
        {/* ── ADDRESS PREVIEW ── */}
        <div className="grid grid-cols-[1fr_2fr] rounded-3xl overflow-hidden min-h-[140px] group"
          style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--border-color)" }}>
          <div className="flex items-center justify-center relative overflow-hidden"
            ...
            <div className={`p-4 rounded-full transition-all duration-500 ${hasAddress ? "shadow-[0_0_20px_rgba(168,85,247,0.3)]" : ""}`}
              style={{ backgroundColor: hasAddress ? "rgba(168,85,247,0.2)" : "var(--border-color)" }}>
              <MapPin size={24} className={hasAddress ? "text-[#a855f7]" : ""}
        ...
        <div className="space-y-6 pt-4">
          <label className="flex items-center gap-3 cursor-pointer group w-fit">
            <div
              ...
              style={formData.mailingSame
                ? { backgroundColor: "#a855f7", border: "1px solid #a855f7" }
            ...
            <span className="text-sm transition-colors" style={{ color: "var(--text-main)", opacity: 0.6 }}>
          ...
          <div className="space-y-3 pt-4">
            ...
            <div className="relative">
              ...
              <Clock size={16} className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none"
        ...
        <section className="pt-6">
          ...
          <div className="space-y-4">
              ...
              {DAYS.map((day) => (
                <button key={day.id} onClick={() => toggleDay(day.id)}
                  ...
                  style={hours[day.id].active
                    ? { backgroundColor: "#a855f7", color: "#ffffff", boxShadow: "0 0 20px rgba(168,85,247,0.2)" }
                    : { color: "var(--text-main)", opacity: 0.3 }
                  }>
                  {day.label}
                </button>
              ))}
            ...
            <div className="space-y-3">
              {DAYS.filter((d) => hours[d.id].active).map((day) => (
                <div key={day.id}
                  ...
                  style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--border-color)" }}>
                  <span className="w-12 text-[12px] font-mono font-bold"
                    style={{ color: "var(--text-main)", opacity: 0.4 }}>
                    {day.label}
                  </span>
                    ...
                    {["open", "close"].map((timeKey) => (
                      <div key={timeKey} className="relative">
                        <input
                          ...
                          style={{ backgroundColor: "var(--bg-main)", border: "1px solid var(--border-color)", color: "var(--text-main)", opacity: 0.7 }}
                          onChange={(e) => {
                            const newHours = {
                              ...hours,
                              [day.id]: { ...hours[day.id], [timeKey]: e.target.value },
                            };
                            updateForm({ operatingHours: newHours });
                          }}
                        />
                        <Clock size={12} className="absolute right-3 top-1/2 -translate-y-1/2"
                  ...
                  <button
                    ...
                    style={{ backgroundColor: "var(--border-color)", color: "var(--text-main)", opacity: 0.5 }}>
                    <Plus size={18} />
                  </button>
                </div>
              ))}
            ...
            <p className="text-[11px] text-center mt-4 uppercase tracking-widest"
              style={{ color: "var(--text-main)", opacity: 0.2 }}>
